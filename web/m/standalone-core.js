...
(function (root) {
  ...
  function compileDraft(workPayload, options) {
    ...
    if (opts.generate_prefs) {
      const stamped = applyGeneratePrefs(comment, opts.generate_prefs);
      Object.keys(stamped).forEach((key) => { comment[key] = stamped[key]; });
    }
    const workId = String(work.work_id || work.id || "");
    const draft = {
  ...
  const GEN_MODELS = [
    ...
    { id: "nai-diffusion-4-full", label: "V4 Full" },
    { id: "nai-diffusion-4-curated", label: "V4 Curated" },
    { id: "nai-diffusion-3", label: "V3" },
    { id: "nai-diffusion-furry-3", label: "V3 Furry" },
  ];
  ...
  const GEN_NOISE = ["karras", "native", "exponential"];
  const MAX_PAID_STEPS = 50;

  function defaultGeneratePrefs() {
    return {
      followModel: true,
      followSize: true,
  ...
  function knownModel(id) {
    ...
    if (GEN_MODELS.some((item) => item.id === model)) return model;
    if (model.indexOf("nai-diffusion-") === 0) return model;
    return "";
  }

  function normalizeGeneratePrefs(raw) {
    const base = defaultGeneratePrefs();
    const incoming = raw && typeof raw === "object" ? raw : {};
    const model = knownModel(incoming.model) || base.model;
    const sampler = GEN_SAMPLERS.indexOf(String(incoming.sampler || "")) >= 0
      ? String(incoming.sampler)
      : base.sampler;
    const noise = GEN_NOISE.indexOf(String(incoming.noise_schedule || "")) >= 0
      ? String(incoming.noise_schedule)
      : base.noise_schedule;
    const seedRaw = incoming.seed == null ? "" : String(incoming.seed).trim();
    return {
      followModel: incoming.followModel !== false && incoming.follow_model !== false,
      followSize: incoming.followSize !== false && incoming.follow_size !== false,
      model: model,
      width: clampInt(incoming.width, base.width, 64, 2048),
      height: clampInt(incoming.height, base.height, 64, 2048),
      steps: clampInt(incoming.steps, base.steps, 1, MAX_PAID_STEPS),
      scale: clampNum(incoming.scale, base.scale, 1, 10),
      sampler: sampler,
      noise_schedule: noise,
      cfg_rescale: clampNum(incoming.cfg_rescale, base.cfg_rescale, 0, 1),
      seed: seedRaw,
      qualityToggle: incoming.qualityToggle !== false,
      autoSmea: !!incoming.autoSmea,
      force_free: incoming.force_free !== false,
    };
  }

  function modelLabel(id) {
    const model = String(id || "").trim();
    const hit = GEN_MODELS.find((item) => item.id === model);
    return hit ? hit.label : (model || "跟随原图");
  }

  function sizePresetId(width, height) {
    const w = Number(width) || 0;
    const h = Number(height) || 0;
    const hit = GEN_SIZES.find((item) => item.width === w && item.height === h);
    return hit ? hit.id : "";
  }

  function applyGeneratePrefs(comment, prefs) {
    const patched = JSON.parse(JSON.stringify(comment || {}));
    const next = normalizeGeneratePrefs(prefs);
    if (!next.followModel) {
      patched.model = next.model;
      patched.Source = next.model;
    }
    if (!next.followSize) {
      patched.width = next.width;
      patched.height = next.height;
    }
    patched.steps = next.steps;
    patched.scale = next.scale;
    patched.sampler = next.sampler;
    patched.noise_schedule = next.noise_schedule;
    patched.cfg_rescale = next.cfg_rescale;
    patched.qualityToggle = next.qualityToggle;
    patched.autoSmea = next.autoSmea;
    if (next.seed !== "") {
      const seedValue = Number(next.seed);
  ...
  function inferModel(source, explicit) {
    ...
    if (lower.indexOf("furry") >= 0) return "nai-diffusion-furry-3";
    if (value.indexOf("V4.5") >= 0 || lower.indexOf("v4.5") >= 0) {
      return lower.indexOf("curated") >= 0 ? "nai-diffusion-4-5-curated" : "nai-diffusion-4-5-full";
    }
    if (value.indexOf("V4") >= 0 || lower.indexOf("v4") >= 0) {
      return lower.indexOf("curated") >= 0 ? "nai-diffusion-4-curated" : "nai-diffusion-4-full";
    }
    if (value.indexOf("V3") >= 0 || lower.indexOf("v3") >= 0 || lower.indexOf("nai-diffusion-3") >= 0) {
  ...
  function studioConfig() {
    return {
      ok: true,
      models: GEN_MODELS.slice(),
      size_presets: GEN_SIZES.slice(),
      samplers: GEN_SAMPLERS.slice(),
      noise_schedules: GEN_NOISE.slice(),
      defaults: defaultGeneratePrefs(),
    };
  }
  ...
  function buildGeneratePayload(patchedComment, forceFree) {
    ...
    if (enforceFree) {
      ...
      steps = Math.max(1, Math.min(MAX_PAID_STEPS, steps));
    }
  ...
  function applyDraftEdits(comment, edits) {
    ...
    if (incoming.steps != null && String(incoming.steps).trim() !== "") {
      patched.steps = Math.max(1, Math.min(MAX_PAID_STEPS, Number(incoming.steps) || 28));
    }
    if (incoming.model != null && String(incoming.model).trim() !== "") {
      patched.model = inferModel(incoming.Source, incoming.model);
      patched.Source = patched.model;
    }
    if (incoming.width != null && String(incoming.width).trim() !== "") {
      patched.width = Math.max(64, Math.min(2048, Number(incoming.width) || patched.width || 832));
    }
    if (incoming.height != null && String(incoming.height).trim() !== "") {
      patched.height = Math.max(64, Math.min(2048, Number(incoming.height) || patched.height || 1216));
    }
    if (incoming.scale != null && String(incoming.scale).trim() !== "") {
      patched.scale = Math.max(1, Math.min(10, Number(incoming.scale) || 5));
    }
    if (incoming.sampler != null && String(incoming.sampler).trim() !== "") {
      patched.sampler = String(incoming.sampler);
    }
    if (incoming.noise_schedule != null && String(incoming.noise_schedule).trim() !== "") {
      patched.noise_schedule = String(incoming.noise_schedule);
    }
    if (incoming.cfg_rescale != null && String(incoming.cfg_rescale).trim() !== "") {
      patched.cfg_rescale = Math.max(0, Math.min(1, Number(incoming.cfg_rescale) || 0));
    }
    if (incoming.qualityToggle != null) patched.qualityToggle = !!incoming.qualityToggle;
    if (incoming.autoSmea != null) patched.autoSmea = !!incoming.autoSmea;
    return patched;
  }
  ...
  const api = {
    ...
    applyGeneratePrefs: applyGeneratePrefs,
    normalizeGeneratePrefs: normalizeGeneratePrefs,
    defaultGeneratePrefs: defaultGeneratePrefs,
