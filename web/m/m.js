  ...
  async function renderBatch(root) {
          ...
          <div class="m-job">
            <div class="m-job-head">
              ...
              <div class="m-row">
                ...
                ${job.retryable ? `<button type="button" class="m-ghost" data-retry="${escapeHtml(job.task_id || "")}">补做未完成</button>` : ""}
    ...
    root.querySelectorAll("[data-retry]").forEach((button) => {
      button.onclick = async () => {
        ...
        if (!taskId) return;
        const job = jobs.find((item) => String(item.task_id || "") === taskId);
        const retryUnknown = !!(job && (job.billing_uncertain || job.status === "unknown"));
        if (!await confirmAction("补做未完成项", retryUnknown
          ? "本组有结果不明的请求。请先核对 NovelAI 记录；确认补做可能再次扣费，已生成的图片会跳过。"
          : "只补做未生成的图片，保留已成功的页和份数。")) return;
        button.disabled = true;
        try {
          const started = await api().post("/api/mobile/queue/" + encodeURIComponent(taskId) + "/retry", { retry_unknown: retryUnknown });
